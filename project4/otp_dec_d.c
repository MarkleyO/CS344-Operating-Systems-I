...
int main(int argc, char *argv[]){
	...
	if(listenSocketFD < 0) error("ERROR opening socket");//if the socket doesn't connect an error is output
	...
	while(1){//continues forever
		...
		if(spawnPID == 0){//contains the code for the child process
			...
			if(atoi(buffer) == 1){//if the handshake is the proper value then program continues
...
//receives the cipher and key from the client
void receiveFile(int estConnect, char** outputTo){
	...
	do{
		...
		i++;//index incremented
	}while(strstr(adjustableBuffer, "\n") == NULL);//this repeats until a packet with the newline character is received
	//////////////////////////////////RECEIVING AND STORING PACKETS//////////////////////////////////////////////////////////////////
...
//creates a new decoded string with cipher and key that is returned
char* encodeString(char* ptString, char* keyString){
	//////////////////////////////////DECLARING / INITIALIZING VARIABLES//////////////////////////////////////////////////////////////
	...
	for(i=0; i<strlen(ptString); i++){//loops for size of plaintext string
		...
		wew = myMod((char1-char2), 27);//default mod function did not work for negative numbers
...
//modulo does not work on negative numbers, so I used a funciton from stack overflow
int myMod(int a, int b){
	//////////////////////////////////COMPUTE MODULO OPERATION////////////////////////////////////////////////////////////////////////////////
	int computed = a % b;//taken from stack overflow
	if(computed < 0)
		computed += b;
	return computed;
	//////////////////////////////////COMPUTE MODULO OPERATION////////////////////////////////////////////////////////////////////////////////
